...
const createSection = (
	...
	for (const key in attributes) {
		if (attributes.hasOwnProperty(key)) {
			section.setAttribute(key, attributes[key]);
		}
	}
	return section;
};
...
const createList = (className: string, elements: string[] = []) => {
...
const createMenu = (): HTMLElement => {
	const menuList = createList("headerList__mobile", [
		"FEATURES",
		"PRICING",
		"CONTACT",
	]);
	const menuLoginButton = createButton("Login");
	const svgMenu = createSVGElement(svgLogo, {
		width: "148",
		height: "25",
		xmlns: "http://www.w3.org/2000/svg",
	});
	const menuCloseButton = createButton(
		`<img src="https://www.svgrepo.com/show/500512/close-bold.svg"/>`
	);
	const menuHeader = createDiv("headerMenu__mobile", [
		svgMenu,
		menuCloseButton,
	]);
	const menuMain = createDiv("mainMenu__mobile", [menuList, menuLoginButton]);
	const facebookIcon = createElement("img", "", "", { src: Facebook });
	const twitterIcon = createElement("img", "", "", { src: Twitter });
	const socialMediaContainer = createDiv("socialMedia__container", [
		facebookIcon,
		twitterIcon,
	]);
	const menu = createDiv("menu-mobile menu__mobile-hidden", [
		menuHeader,
		menuMain,
...
const createMain = (): HTMLElement => {
	...
	const chromeButton = createButton(
		`<a href="#downloads">Get it on Chrome</a>`
	);
	const firefoxButton = createButton(
		`<a href="#downloads">Get it on Firefox</a>`
	);
	...
	const semiCirclebackgroundTab = createDiv("semicircle");
	const imgContainerTab = createDiv("imgTab__container", [
		imgTab,
		semiCirclebackgroundTab,
	]);
	...
	const downloads = createSection(
		"downloads__container",
		[downloadsInfoContainer, cards],
		{ id: "downloads" }
	);
...
const createFooter = (): HTMLElement => {
	...
	const footerLogo = createSVGElement(svgLogo, {
		width: "148",
		height: "25",
		xmlns: "http://www.w3.org/2000/svg",
	});
	const footerList = createList("footer__list", [
		"FEATURES",
		"PRICING",
		"CONTACT",
	]);
	...
	footer.append(footerLogo, footerList, socialMediaContainer);
	return footer;
};
